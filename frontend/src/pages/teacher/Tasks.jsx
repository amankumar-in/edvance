...
function Tasks() {
  ...
  if (isTasksError) {
    return (
      <div className="space-y-6">
        <TaskPageHeader />
        <ErrorCallout errorMessage={tasksError?.response?.data?.message || tasksError?.message || 'Failed to load tasks'}
                      ...
                      <DropdownMenu.Content variant='soft'>
                        <DropdownMenu.Group >
                          ...
                          <DropdownMenu.Item asChild>
                            <Link to={`/teacher/tasks/edit/${task._id}`}>
                              <PencilIcon size={14} />
                              Edit Task
                            </Link>
                          </DropdownMenu.Item>
...
function TaskPageHeader() {
  return (
    <PageHeader title='Tasks' description='Manage tasks for your classes' >
      <CreateTaskButton />
    </PageHeader>
  )
}
