...
import { useGetStudentTransactionSummary } from '../../../api/points/points.queries';
...
const OverviewTab = ({
  ...
  const { data, isLoading, isError, error } = useGetStudentTransactionSummary(studentId);
  const earnedSummary = data?.data?.filter(item => item.type === 'earned')?.[0]
            ...
            <div className="space-y-4">
              {isLoadingTransactions ? (
                ...
                transactions?.map((transaction, index) => (
                  <Card key={transaction._id} className="p-4 transition-all duration-200 hover:shadow-md"
                  >
                    <Flex justify="between" align="center">
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              ) : earnedSummary?.sources?.length > 0 ? (
                earnedSummary?.sources?.map((source) => {
                  const sourcePoints = source.totalPoints;
                  const totalPoints = earnedSummary?.totalPoints || 1;
                  const percent = Number(((sourcePoints / totalPoints) * 100).toFixed(2));
                      ...
                      <Flex align="center" gap="3" className="mb-2">
                        ...
                        <div className="flex-1">
                          <Flex justify="between" align="center" className="mb-1">
                            ...
                            <Text size="2" weight="bold">{sourcePoints}</Text>
                          </Flex>
                          <Progress value={percent} className="mb-1" />
                          <Text size="1" color="gray">{percent}% of total points</Text>
                        </div>
                      </Flex>
                    </div>
                  );
          ...
          {pointAccount.progressPercentage > 0 && <Card className="relative p-6 overflow-hidden" style={{ backgroundColor: 'var(--amber-a2)' }}>
